...
Adafruit_DCMotor *LMotor = AFMS.getMotor(4); // Left Motor
Adafruit_DCMotor *RMotor = AFMS.getMotor(3); // Right Motor
  ...
  if (Serial.available()) {
    ...
    switch(cntr) // Perform an action depending on the command
    {
      ...
      case 'e':
        if(CntrlMode == 1) {
          if (CntrlSide == -1) {
              myservo.write(CntrlSpd);              // tell servo to go to position in variable 'pos'
              delay(10);                       // waits 15ms for the servo to reach the position
          } 
          else if (CntrlSide == 1) {
            RDirNew = CntrlDir;
